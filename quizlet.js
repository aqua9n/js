...
(function() {	
	...
	function initLoad() {
		if (href.includes("quizlet.com")) {
			if (href.includes("/micromatch")) {
				cAlert('<h2>ゲームモード: Micromatch</h2><br>タイマーが0.5で止まります<br><button class="UIButton" id="micromatchButton" type="button"><span class="UIButton-wrapper"><span>Start</span></span></button>');
				...
				cAlert('<p>このゲームモードでは使用できません。urlの後ろ側 「/match」を/「micromatch」変更して下さい</p>');
